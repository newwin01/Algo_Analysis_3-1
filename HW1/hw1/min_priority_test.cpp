...
HW 1
reference: https://github.com/newwin01/Data_Structure/tree/master/practice09_21900628_%EC%9E%A5%EC%84%B8%EC%B0%BD
My git repo used on data structure
...
class MinHeap{
    ...
    public:
        ...
        void insert(string name, int key, string school); //insert the value
        Element minimun(); //Get the minimum value for heap
        void deleteHeap();
        void decrease_Key(int id_value, int key);
        void print();
...
//Insert value maintaining the min heap structure
void MinHeap::insert(string name, int key, string school){
    Element item;
    if(IsFull()){
    ...
    while((i!=1)&&(item.getKey()<heap[i/2].getKey())){//insert the value comparing the its node
        heap[i] = heap[i/2];
        i/=2;
    }
    heap[i]=item;
    
    cout << "New element [" << name << ", " << key << ", " << school << "] is inserted" << endl;
}

Element MinHeap::minimun(){ //Get the root value which is the minimum 
    return heap[1];
}
...
void MinHeap::deleteHeap(){
    ...
    heap[1] = heap[index--]; //decrease the index
    parent = 1;
    while (parent*2<=index){ //while there is no anymore leaf
        if(parent*2+1<=index){ //if there is leaf
            if(heap[parent*2].getKey()<heap[parent*2+1].getKey()) // consider whether leaf is smaller or bigger
                smaller = parent*2;
            else
                smaller = parent*2+1;
        }
            ...
            smaller = parent*2;   
        if(heap[parent].getKey()<=heap[smaller].getKey()) //if the structure is completely formed
            break;
        swap(&heap[parent],&heap[smaller]); //swap the value
        parent = smaller; //set parent the small one
    }

}

void MinHeap::decrease_Key(int id_value, int key){
    int temp = heap[id_value].getKey(); // Check it is increasing or decreasing
    int i = index;
    Element item;
    if(temp<key){
    ...
    else{ //Swap the first one and last one, use insertion algorithm
        heap[id_value].insertKey(key);
        item = heap[id_value];
        // swap(&heap[id_value],&heap[index]);
